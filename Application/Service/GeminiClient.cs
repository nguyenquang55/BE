...
namespace Application.Service
{
    public class GeminiClient : IGeminiClient
    {
        ...
        {
            var primaryKey = _configuration["GEMINI_API_Key:Key"];
            var backupKey = _configuration["GEMINI_API_Key:BackupKey"];

            try
            {
                return await ExecuteGeminiRequest(primaryKey, prompt);
            }
            catch (Exception ex)
            {
                if (!string.IsNullOrEmpty(backupKey))
                {
                    try
                    {
                        return await ExecuteGeminiRequest(backupKey, prompt);
                    }
                    catch (Exception exBackup)
                    {
                        throw new Exception($"All Gemini API attempts failed. Backup error: {exBackup.Message}");
                    }
                }
                throw new Exception($"Gemini API failed: {ex.Message}");
            }
        }

        private async Task<string> ExecuteGeminiRequest(string apiKey, string prompt)
        {
