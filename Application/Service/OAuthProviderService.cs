...
namespace Application.Service
{
    public class OAuthProviderService : IOAuthProviderService
    {
        ...
        public async Task<Result<string>> Refresh(string sessionToken, CancellationToken ct = default)
        {
            string? userId = await ValidateClientAsync(sessionToken);
            if (string.IsNullOrEmpty(userId))
            {
            ...
            var linkedProviders = await _oAuthProviderRepository.FindAsync(p => p.UserId == Guid.Parse(userId));
            if (!linkedProviders.Any())
            {
                return Result<string>.FailureResult("No linked OAuth providers found", string.Empty, System.Net.HttpStatusCode.NotFound);
            }

            foreach (var provider in linkedProviders)
            {
                var refreshToken = await _oAuthTokenRepository.FindAsync(t => t.AuthProviderId == provider.Id && t.UserId == provider.UserId);
                ...
                var tokenEndpoint = _configuration[$"OAuth:{provider.Provider}:TokenEndpoint"];
                var clientId = _configuration[$"OAuth:{provider.Provider}:ClientId"];
                var clientSecret = _configuration[$"OAuth:{provider.Provider}:ClientSecret"];
                ...
                try
                {
                    tokenResponse = await http.PostAsync(tokenEndpoint, new FormUrlEncodedContent(requestContent), ct);
                }
                catch (Exception ex)
                {
                    Console.WriteLine($"Failed to refresh token for provider {provider.Provider}: {ex.Message}");
                    continue;
                }

                if (!tokenResponse.IsSuccessStatusCode)
                {
                    Console.WriteLine($"Token refresh failed for provider {provider.Provider}: {await tokenResponse.Content.ReadAsStringAsync()}");
                    continue;
                }
                ...
                try
                {
                    newToken = JsonSerializer.Deserialize<TokenResponse>(tokenContent);
                }
                catch (Exception ex)
                {
                    Console.WriteLine($"Failed to deserialize token response for provider {provider.Provider}: {ex.Message}");
                    continue;
                }
                ...
                await _redisCacheService.SetAsync($"OAuth:{provider.Provider}:Accesstoken:{userId}", newToken.access_token, TimeSpan.FromSeconds(newToken.expires_in));
            }

            return Result<string>.SuccessResult("Tokens refreshed successfully", "Success", System.Net.HttpStatusCode.OK);
        }
